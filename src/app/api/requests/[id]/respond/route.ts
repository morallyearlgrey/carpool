...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const request: any = await RequestModel.findById(requestId).lean();
    ...
    const driverId = request.requestReceiver?.toString();

    if (!driverId || !riderId) return NextResponse.json({ error: 'invalid request data' }, { status: 400 });
    ...
    if (action === 'accept') {
      ...
      await User.findByIdAndUpdate(driverId, { $push: { rides: ride._id }, $pull: { incomingRequests: requestId } });

      // Remove from rider's outgoingRequests and set user's currentRide
      await User.findByIdAndUpdate(riderId, { $pull: { outgoingRequests: requestId }, $push: { rides: ride._id }, currentRide: ride._id });
